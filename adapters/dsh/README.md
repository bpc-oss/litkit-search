...
DeepSeek Harness (dsh) plugin adapter for [litkit-dsh](../README.md) — registers
`litkit_search`, `litkit_sources`, `litkit_download`, `litkit_verify`, and
`litkit_doctor` tools in any dsh profile.

## Prerequisites

- The `litkit-dsh` CLI must be installed and on the PATH of the dsh process:
  `pip install litkit-search` (the Python wheel). Run `litkit-dsh doctor` to check.
...
- The adapter shells out to `litkit-dsh <sub> --json` (zero core changes, clean
