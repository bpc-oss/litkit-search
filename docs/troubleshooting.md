...
| `litkit-dsh` command not found after install | Scripts dir not on PATH — use `python -m litkit-dsh` or add `%LOCALAPPDATA%\Programs\Python\Python311\Scripts` to PATH |

## Runtime

| Symptom | Fix |
|---|---|
| `litkit-dsh doctor` shows core-dependency FAIL | Reinstall: `pip install --force-reinstall litkit-search` |
| Search returns nothing for a source | That source may be rate-limited or temporarily down — `litkit-dsh sources` shows status; retry later or narrow `-s` |
...
| Publisher PDF downloads fail | Try `litkit-dsh download` with browser chain installed (see [browser.md](browser.md)) |
